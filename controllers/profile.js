...
const getProfile = async (req, res, next) => {
	try {
		...
		if (!foundProfile) {
			return next(new HttpError('There is no profile of the current user', 400));
		}
...
const getProfileByUserId = async (req, res, next) => {
	try {
		...
		if (err.kind == 'ObjectId') return next(new HttpError('Profile not found', 400));
		next(new HttpError(err.message, 500));
	}
};
...
const deleteProfileEducation = async (req, res, next) => {
	try {
		const profile = await Profile.findOne({ user: req.user.id });
		profile.education = profile.education.filter((edu) => edu._id.toString() !== req.params.education_id);
